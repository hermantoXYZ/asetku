...
from .models import User, Kategori, AsetBaru, DetailAset, Pembelian, Lampiran, PenanggungJawab, PosisiAsset
...
class AsetBaruForm(forms.ModelForm):
    class Meta:
        model = AsetBaru
        fields = ['nama_aset', 'kode_aset', 'kategori', 'kondisi_aset', 'posisi_aset']
...
class PembelianForm(forms.ModelForm):
    tanggal_kontrak = forms.DateField(widget=forms.DateInput(attrs={'type': 'date'}))
    ...
    class Meta:
        model = Pembelian
        fields = ['tanggal_kontrak', 'toko_distributor', 'no_kontrak', 'unit', 'harga_satuan', 'harga_total']
...
class PosisiAsetForm(forms.ModelForm):
    class Meta:
        model = PosisiAsset
        fields = ['lokasi']